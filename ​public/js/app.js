...
function setupEvents() {
  ...
  document.querySelectorAll(".stat-card").forEach(card => {
    card.addEventListener("click", () => {
      ...
      if (filter === "get") {
        renderAccordion(apiData.filter(a => (a.method || "GET").toUpperCase() === "GET"));
      } else if (filter === "post") {
        renderAccordion(apiData.filter(a => (a.method || "").toUpperCase() === "POST"));
      } else {
        renderAccordion(apiData);
      }
    });
  ...
  document.getElementById("search-input").addEventListener("input", (e) => {
    ...
    const filtered = apiData.filter(a => a.name.toLowerCase().includes(q) || (a.category && a.category.toLowerCase().includes(q)));
